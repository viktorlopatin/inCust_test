...
#Показать событие button
async def show_event_hundler_button(callback_query: types.CallbackQuery) :
	callback_data = str(callback_query.message.reply_markup.inline_keyboard[0][0].callback_data).split(".")
	event = await db.getEvent(callback_data[1])
	await send_message_with_media(callback_query.message, event, False)
